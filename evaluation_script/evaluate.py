...
def main() -> None:
    ...
    # Modified: add a flag to check if logging to W&B is needed
    parser.add_argument(
        "--log_to_wb",
        action="store_true",
    ...
    # Modified: use './wbapi.json' to log to W&B
    if args.log_to_wb:
        ...
        try:
            with open(file_path, 'r') as file:
                ...
                api = data["key"]
                wandb.login(key=api)
            wandb.init(project="detox", name="backtranslation_baseline_eval", entity="speech_sanitizers")
    ...
    # Modified: to log official evaluation metrics
    ...
    # Modified: log results and end the W&B run, if --log_to_wb was specified and wandb session was started
    if args.log_to_wb:
        wandb.log(result)
        wandb.finish()
